...
app/
  [locale]/
    ...
    page.tsx            home: split hero, trust strip, departments, emergency band
    patients/           Patients & Visitors (phase 4)
    academics/          Medical College (phase 4)
    about/  news/  contact/
    not-found.tsx
  api/health/route.ts   liveness probe (Railway healthcheck)
  page.tsx              root → redirects to default locale
  globals.css           Tailwind v4 + OKLCH design tokens
components/             site-header, site-footer, locale-switcher
content/messages/       en.json, hi.json (parity-tested)
i18n/                   routing, request config
lib/                    fonts, site metadata
...
Railway uses `Dockerfile` directly (the `runner` stage). Healthcheck path is `/api/health`.
...
PAYLOAD_SECRET          (rotate per environment, phase 5)
